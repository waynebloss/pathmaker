 ...
 */
function PathMaker(basePath, opts) {
  /** 
   * Directory delimiter. ('/')
   * @type {string}
   */
  ...
  function combinePath(path1, path2) {
    ...
    var d1 = path1.endsWith(delimiter);
    var d2 = path2.startsWith(delimiter);
    if (d1 && d2) return path1 + path2.substr(1);
    if (d1 || d2) return path1 + path2;
  ...
  function createSub(subPath, subOpts) {
    ...
    if (isPathString(subOpts)) pm.path = subOpts;else if (subOpts && isPathString(subOpts.path)) pm.path = subOpts.path;
    return pm;
  }
  /**
  ...
  function mergePayload(path, payload) {
    var parts = path.split(delimiter).map(
    /**
    ...
    function partFromPayload(part) {
      if (part.length < 2 || part.charAt(0) !== tokenPrefix) return part;
      var key = part.substr(1);
      if (!payload.hasOwnProperty(key)) return part;
      var value = payload[key];
      if (value === undefined || value === null) return '' + value; // Returns 'undefined' or 'null'.
